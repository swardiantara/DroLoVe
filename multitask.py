...
from utils.preprocessing import BaselineDataset, MultitaskDataset, assert_data_size, label2multitask110
from utils.postprocessing import visualize_projection, label2class, visualize_prediction
...
def main():
    ...
    print('[multitask-110] - Current Workdir: ', workdir)
    ...
    train_df = pd.read_csv(train_path)
    train_df['multi_task_label'] = train_df["label"].apply(label2multitask110)
    test_df = pd.read_csv(test_path)
    test_df['multi_task_label'] = test_df["label"].apply(label2multitask110)
    ...
    model = DroneLog(bert_model, encoder_type,
                                n_heads, n_layers, freeze_embedding, bidirectional, lstm_hidden_size, pooling, exclude_cls_before, exclude_cls_after, num_classes_multiclass, None).to(device)
    ...
    print(f"[multitask-110] - {train_started_at} - Start Training...\n")
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            for logits in logits_multiclass:
                ...
                string_label = label2class([1 if element >= 0.5 else 0 for element in after_sigmoid])
    ...
    best_model.eval()
    label_order = ['high', 'low','medium', 'normal']
    ...
    print(f"\n[multitask-110] - {eval_started_at} - Start evaluation...\n")
    with torch.no_grad():
        for batch in test_loader:
            input_ids = batch["input_ids"].to(device)
            attention_mask = batch["attention_mask"].to(device)
    ...
    prediction_df["prob"] = list(pred_probs_multiclass)
    ...
    print(f"[multitask-110] - {eval_finished_at} - Finish...\n")
